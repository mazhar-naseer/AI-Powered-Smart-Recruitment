...
function InsightReport({application,onRetryAI,retryingAI}:{application:Application;onRetryAI:(id:string)=>void;retryingAI?:string}){
  ...
  const deterministic=scores.deterministic_total??application.deterministic_score??KPI.reduce((total,kpi)=>total+(scores[kpi.key]||0)*kpi.weight/100,0);
  ...
  return <div className="insight-report">
    ...
    <div className="kpi-grid">{KPI.map(kpi=>{const raw=scores[kpi.key]||0;const contribution=raw*kpi.weight/100;return <article className="kpi-card" key={kpi.key}><div className="kpi-title"><span>{kpi.label}</span><strong>{raw.toFixed(1)}%</strong></div><div className="kpi-bar"><i style={{width:`${Math.min(100,Math.max(0,raw))}%`}}/></div><div className="kpi-math"><span>Weight {kpi.weight}%</span><b>+{contribution.toFixed(2)} points</b></div><p>{kpi.detail}</p></article>})}</div>
    <div className="calculation"><strong>Score calculation</strong><code>{KPI.map(kpi=>`${(scores[kpi.key]||0).toFixed(2)} × ${kpi.weight}%`).join(' + ')} = {deterministic.toFixed(2)}%</code>{geminiScore!=null?<div><p>Gemini assessment: {geminiScore.toFixed(2)}%. Final score = {deterministicWeight.toFixed(1)}% deterministic + {aiWeight.toFixed(1)}% Gemini. AI influence is confidence-adjusted.</p>{aiConfidence!=null&&<p>Gemini confidence: {aiConfidence.toFixed(1)}% · Engine difference: {scoreDifference?.toFixed(1)} points.</p>}{manualReview&&<p><strong>Guardrail triggered:</strong> The engines disagree significantly, so an employer must manually review this result.</p>}</div>:<div><p>Deterministic score is complete. Gemini status: <strong>{application.ai_status||'not configured'}</strong>.</p>{application.ai_status==='failed'&&<><p className="muted">{application.ai_error||'The Gemini request did not complete.'}</p><button className="button tiny" disabled={retryingAI===application.id} onClick={()=>onRetryAI(application.id)}>{retryingAI===application.id?'Retrying Gemini…':'Retry Gemini analysis'}</button></>}</div>}</div>
    ...
    <div className="insight-note"><strong>Important:</strong> Matching every required skill does not equal a 100% overall match. Skill coverage contributes 30%; the remaining 70% measures full-resume relevance, role alignment, and experience evidence.</div>
  </div>
}
