import{Fragment,useEffect,useState,type FormEvent}from'react';import{Link,useParams}from'react-router-dom';import{BriefcaseBusiness,CheckCircle2,ChevronDown,ChevronUp,Inbox,MapPin,Search,SearchX,UploadCloud}from'lucide-react';import{request}from'../api';import{aiEngineLabel,aiVersionLabel}from'../ai';import{Badge,Empty,Spinner}from'../components';import type{Application,Job}from'../types';
...
function InsightReport({application,onRetryAI,retryingAI}:{application:Application;onRetryAI:(id:string)=>void;retryingAI?:string}){
  ...
  // The component-score payload has always carried the AI engine's own semantic
  // reading under its original key; it is a stored field name, never displayed.
  const aiScore=application.ai_score??scores.gemini_semantic;
  ...
  return <div className="insight-report">
    ...
    <div className="calculation"><strong>Score calculation</strong><code>{KPI.map(kpi=>`${Number(scores[kpi.key]||0).toFixed(2)} × ${Number(weights[kpi.key]||0)}%`).join(' + ')} = {deterministic.toFixed(2)}%</code>{aiScore!=null?<div><p>SmartHire AI assessment: {aiScore.toFixed(2)}%. Final score = {deterministicWeight.toFixed(1)}% deterministic + {aiWeight.toFixed(1)}% SmartHire AI. AI influence is confidence-adjusted.</p>{aiConfidence!=null&&<p>SmartHire AI confidence: {aiConfidence.toFixed(1)}% · Engine difference: {scoreDifference?.toFixed(1)} points.</p>}{manualReview&&<p><strong>Guardrail triggered:</strong> The engines disagree significantly, so an employer must manually review this result.</p>}</div>:<div><p>Deterministic score is complete. SmartHire AI status: <strong>{application.ai_status||'not configured'}</strong>.</p>{application.ai_status==='failed'&&<><p className="muted">{application.ai_error||'The SmartHire AI request did not complete.'}</p><button className="button tiny" disabled={retryingAI===application.id} onClick={()=>onRetryAI(application.id)}>{retryingAI===application.id?'Retrying SmartHire AI…':'Retry SmartHire AI analysis'}</button></>}</div>}</div>
    ...
    <div className="evidence-grid"><section><h4>Matched evidence</h4><div className="chips">{application.matched_skills.length?application.matched_skills.map(skill=><Badge key={skill} tone="green">{skill}</Badge>):<span className="muted">No required skills detected</span>}</div></section><section><h4>Missing or weak evidence</h4><div className="chips">{missing.length?missing.map(skill=><Badge key={skill} tone="orange">{skill}</Badge>):<span className="muted">All required skills were detected</span>}</div></section><section><h4>Engine and recommendation</h4><p><strong>{aiEngineLabel(application.ai_provider,'Processing')}</strong></p><p className="muted">SmartHire AI recommendation: {application.ai_recommendation?.replaceAll('_',' ')||'Pending'}</p></section></div>
    <div className="insight-note"><strong>Audit trail:</strong> Analysis {application.analysis_version||'legacy'} · Parser {application.parser_version||'legacy'} · Prompt {aiVersionLabel(scores.prompt_version)}. Matching every skill alone does not produce 100%; the employer controls the six-part scorecard.</div>
  </div>
}
