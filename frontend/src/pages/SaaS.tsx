...
export function WorkspaceCommercial(){
  ...
  async function plan(plan_key:string){
    ...
    try{await request('/workspace/billing/change-plan',{method:'POST',body:JSON.stringify({plan_key})});const confirmation='Plan request submitted. An administrator must approve it before activation.';setMessage(confirmation);window.alert(confirmation);await load()}
...
export function AdminCommercial(){
  const[data,setData]=useState<any>();const[requests,setRequests]=useState<any[]>();const[query,setQuery]=useState('');const[status,setStatus]=useState('all');useEffect(()=>{request('/admin/saas/overview').then(setData);request<any[]>('/admin/saas/plan-requests').then(setRequests)},[]);if(!data||!requests)return <Spinner/>;
  ...
  return <div className="saas-admin">
    <section className="saas-admin-hero"><div><span>COMMERCIAL OPERATIONS</span><h1>Subscription command center</h1><p>Monitor tenant health, revenue readiness, and plan adoption across the SmartHire platform.</p></div><div className="saas-hero-signal"><i/><div><strong>{requests.filter(item=>item.status==='pending').length} plan requests pending</strong><span><Link to="/admin/saas/requests">Review plan requests</Link></span></div></div></section>
