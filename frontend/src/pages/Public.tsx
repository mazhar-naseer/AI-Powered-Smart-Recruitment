// @ts-nocheck
import{useState,type FormEvent}from'react';import{Link,Navigate,useNavigate}from'react-router-dom';import{ArrowLeft,ArrowRight,BrainCircuit,Briefcase,BriefcaseBusiness,CheckCircle2,Clock3,FileText,Handshake,LockKeyhole,ShieldCheck,SquareCode,UserRound,Users}from'lucide-react';import{Logo}from'../components';import{useAuth}from'../auth';
export function Landing(){return <div className="landing"><nav className="landing-nav"><Logo/><div><a href="#features">Features</a><a href="#roles">For Employers</a><a href="#roles">For Applicants</a><Link className="button small" to="/register">Get Started</Link></div></nav><section className="hero"><div><h1>Smart Hiring.<br/>Better Matches.<br/><em>Stronger Teams.</em></h1><p>AI-powered resume screening that connects the right talent with the right opportunities.</p><div className="hero-actions"><Link className="button" to="/register?role=employer">I’m an Employer</Link><Link className="button outline" to="/register?role=applicant">I’m an Applicant</Link></div></div><div className="hero-art"><div className="resume-card"><div className="avatar"><UserRound/></div><div className="lines"/><strong>95%<small>Match</small></strong></div><div className="people" aria-hidden="true"><span><SquareCode/></span><span><Handshake/></span><span><Briefcase/></span></div></div></section><section id="features" className="features"><h2>Why SmartHire?</h2><div className="feature-grid">{[[BrainCircuit,'Smart Resume Screening','Our engine analyzes resumes and ranks candidates by best match.'],[Clock3,'Save Valuable Time','Reduce manual effort and focus on the best candidates.'],[Users,'Better Quality Hire','Find talent whose skills match your actual needs.'],[LockKeyhole,'Secure & Reliable','Private files and role-based access protect every step.']].map(([Icon,title,body])=><article key={String(title)}><Icon/><h3>{title}</h3><p>{body}</p><CheckCircle2 size={16}/></article>)}</div></section><section className="numbers"><div><strong>500+</strong><span>Companies</span></div><div><strong>10K+</strong><span>Job Seekers</span></div><div><strong>25K+</strong><span>Resumes Processed</span></div><div><strong>98%</strong><span>Satisfaction Rate</span></div></section></div>}
export function Login(){const{login}=useAuth();const nav=useNavigate();const registered=new URLSearchParams(location.search).get('registered')==='1';const[email,setEmail]=useState('');const[password,setPassword]=useState('');const[error,setError]=useState('');const[busy,setBusy]=useState(false);async function submit(e:FormEvent){e.preventDefault();setBusy(true);setError('');try{const user=await login(email,password);nav(user.role==='employer'?'/employer':user.role==='admin'?'/admin':'/applicant/jobs')}catch(err){setError(err instanceof Error?err.message:'Login failed')}finally{setBusy(false)}}return <div className="auth-page"><div className="auth-side"><Logo/><div><h1>Welcome back!</h1><p>Login to continue your hiring or job search journey.</p><div className="auth-illustration" aria-hidden="true"><span><LockKeyhole/></span><span><FileText/></span><span><BriefcaseBusiness/></span></div></div></div><form className="auth-card" onSubmit={submit}><h2>Login</h2><p>Enter your account credentials.</p>{registered&&!error&&<div className="alert success">Account created. Log in to continue.</div>}{error&&<div className="alert error">{error}</div>}<label>Email Address<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" required/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} minLength={8} required/></label><button className="button full" disabled={busy}>{busy?'Signing in…':'Login'}<ArrowRight size={17}/></button><p className="center">Don’t have an account? <Link to="/register">Register here</Link></p></form></div>}
...
export function AdminLogin(){
  ...
  return <div className="admin-login-page">
    ...
    <form className="admin-login-card" onSubmit={submit}>
      ...
      <Link className="admin-back" to="/login"><ArrowLeft size={16}/>Return to SmartHire application</Link>
    </form>
  </div>
}
