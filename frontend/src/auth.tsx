...
type Auth={user:User|null;loading:boolean;login:(e:string,p:string)=>Promise<User>;register:(v:Record<string,string>)=>Promise<Registration>;verifyEmail:(v:{email?:string;code?:string;token?:string})=>Promise<User>;exchangeOAuth:(code:string)=>Promise<User>;updateUser:(user:User)=>void;logout:()=>Promise<void>};
...
const exchangeOAuth=async(code:string)=>acceptAuth(await request<AuthData>('/auth/oauth/exchange',{method:'POST',auth:false,body:JSON.stringify({code})}));
...
return <Context.Provider value={{user,loading,login,register,verifyEmail,exchangeOAuth,updateUser:setUser,logout}}>{children}</Context.Provider>}
